...
def main():
    ...
    model = build_attnresunet()

    model.compile(optimizer=Adam(learning_rate=LEARNING_RATE),
                  loss=dice_bce_loss,
                  metrics=["accuracy", tf.keras.metrics.MeanIoU(num_classes=2)])

    run_name = f"attnresunet_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}"
    callbacks = [
    ...
    print(f"✅ AttnResUNet model saved to {CHECKPOINT_DIR}")
